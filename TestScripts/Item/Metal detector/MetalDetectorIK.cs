...
public class MetalDetectorIK : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // 1. Считаем инерцию и махание
        swayModule.CalculateSway(scannerModule.IsActive);
        ...
        // 4. ВЫЧИСЛЕНИЕ ПОЗИЦИИ КАТУШКИ
        Vector3 rawLocalPos;
        Vector3 targetNormal = Vector3.up;

        // ИСПРАВЛЕНИЕ: Вычисляем идеальную позицию катушки на текущей дуге взмаха
        ...
        Vector3 expectedAirLocalPos = coilTransform.parent.InverseTransformPoint(laggedWorldCoilPos);
        ...
        {
            rawLocalPos = expectedAirLocalPos;
            targetNormal = Vector3.up;
        }

        // 5. СГЛАЖИВАНИЕ ПОЗИЦИИ И НОРМАЛИ
        ...
        // ИСПРАВЛЕНИЕ: Лимит теперь считается от текущей точки дуги (expectedAirLocalPos), а не от статичного центра
        finalLocalPos.z = Mathf.Max(finalLocalPos.z, expectedAirLocalPos.z - maxBackwardLimit);

        // 6. ПРИМЕНЕНИЕ ПОЗИЦИЙ
