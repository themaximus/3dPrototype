...
public class EquippableItemController : MonoBehaviour
{
    ...
    private void HandleConsume()
    {
        ...
        if (cData != null)
        {
            // Ищем игрока от камеры
            GameObject player = mainCam != null ? mainCam.transform.root.gameObject : null;
            if (player != null)
            {
                // Применяем эффект (лечение и тд)
                cData.Use(player);

                // Пытаемся найти инвентарь на самом игроке или его дочерних объектах
                InventorySystem invSystem = player.GetComponentInChildren<InventorySystem>();

                // Если по какой-то причине он не в детях, ищем по всей сцене
                if (invSystem == null)
                {
                ...
                if (invSystem != null)
                {
                    ...
                    Debug.Log($"[EquippableItemController] Предмет {itemData.itemName} удален из активного слота.");
                }
                else
                {
                    Debug.LogError("[EquippableItemController] Не найден InventorySystem! Предмет не удален.");
                }
            }
        }
    }
