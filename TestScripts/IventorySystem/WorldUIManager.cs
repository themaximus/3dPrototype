...
public class WorldUIManager : MonoBehaviour
{
    ...
    public void Show(ItemPickup item)
    {
        ...
        // Заполняем тексты
        nameText.text = item.itemData != null ? item.itemData.itemName : "Неизвестный предмет";

        // Берем описание: сначала проверяем customDescription, если пусто - берем из itemData
        string desc = "";
        if (!string.IsNullOrEmpty(item.customDescription))
        {
            desc = item.customDescription;
        }
        else if (item.itemData != null && !string.IsNullOrEmpty(item.itemData.description))
        {
            desc = item.itemData.description;
        }
        else
        {
            desc = "Нет описания";
