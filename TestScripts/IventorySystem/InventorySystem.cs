...
public class InventorySystem : MonoBehaviour
{
    ...
    // Метод для удаления одной единицы предмета из текущего активного слота
    public void RemoveActiveItem()
    {
        if (activeQuickSlotIndex < 0 || activeQuickSlotIndex >= quickSlots.Count) return;

        InventorySlot slot = quickSlots[activeQuickSlotIndex];
        if (!slot.IsEmpty())
        {
            slot.RemoveQuantity(1);
            OnQuickSlotUpdated?.Invoke(activeQuickSlotIndex);

            // Если предметы в слоте кончились, убираем объект из рук
            if (slot.IsEmpty())
            {
                EquipItemFromSlot(activeQuickSlotIndex);
            }
    ...
    /// <summary>
    /// Вызывается из QuickSlotInputHandler (Правая кнопка мыши)
    /// </summary>
